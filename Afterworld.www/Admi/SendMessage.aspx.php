...
<div><span class="bold">Banned:</span> <?= (!empty($searchResult['IsBanned']) && $searchResult['IsBanned'] == 1) ? 'Yes' : 'No' ?></div>
                <div><button onclick="openModal()">Send Message</button></div>
            </div>
        </div>
    </div>
...
<div id="editUserModal" class="modal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="/api/v1/send-personal-message" method="POST" class="modal-body">
        <label for="Userid">Userid:</label>
        <input type="text" name="userid" value={$userid} class="hidden" disabled>

        <label for="Subject">Subject:</label>
        <input type="text" placeholder="Hello!" id="Subject" name="Subject" required><?= htmlspecialchars($searchResult['Content'] ?? '') ?>

        <label for="content">Content:</label>
        <textarea id="content" placeholder="From the AFTERWORLD DEV Team!" name="content" rows="5"><?= htmlspecialchars($searchResult['Content'] ?? '') ?></textarea>
