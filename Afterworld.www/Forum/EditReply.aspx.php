...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("SELECT PostId FROM forum_replies WHERE id = ?");
...
$stmt = $pdo->prepare("SELECT Content FROM forum_replies WHERE id = ?");
$stmt->execute([$replyId]);
...
<form method="post">
    ...
    <textarea name="content" rows="10" cols="70" required><?= htmlspecialchars($reply['Content']) ?></textarea><br><br>
