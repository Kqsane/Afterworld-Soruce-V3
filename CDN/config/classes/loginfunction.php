...
// no need for a rewrite, this is already secured a bit
...
function signup(string $username,string $password,string $gender) {
	...
	$insertQuery = $pdo->prepare(
		"INSERT INTO users (Username, Description, isAdmin, JoinData, Password, Gender, ROBLOSECURITY, IsBanned, CharApp, activated, discord_id) 
		VALUES (:username, :description, :isAdmin, :jointime, :password, :gender, :token, :isBanned, '', 0, NULL)"
	);
...
function logout(){
	...
	setcookie(".ROBLOSECURITY","",-1, "/", "194.62.248.75");
	return true;
}
